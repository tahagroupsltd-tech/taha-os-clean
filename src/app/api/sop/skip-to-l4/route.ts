...
const L1_TO_L3_STEPS = [
  ...
  // L3
  { level: 3, key: 'alignment_call' },
  { level: 3, key: 'scripts_complete' },
]
...
export async function POST(req: NextRequest) {
  ...
  // 2. Insert missing progress entries for L1-L3
  const insertedProgress: any[] = []
  for (const step of L1_TO_L3_STEPS) {
  ...
  // 3. Initialize Level 4 SOP (create tasks/events, update project's level to 4)
  ...
  for (const task of blueprint.tasks) {
    try {
      ...
      console.error('Failed to create L4 SOP task:', e)
    }
  }
  ...
  for (const event of blueprint.events) {
    try {
      ...
      console.error('Failed to create L4 SOP event:', e)
    }
  }
  ...
  return NextResponse.json({
    ...
    summary: `Skipped to L4 SOP for "${project.name}" (Existing Client). L1-L3 marked complete. L4 SOP activated with ${createdTasks.length} tasks and ${createdEvents.length} event(s).`,
  })
}
