      ...
      {showForm && (
        <div className="fixed inset-0 bg-black/40 z-50 flex items-center justify-center p-3">
          <div className="bg-white rounded-xl shadow-xl w-full max-w-lg flex flex-col max-h-[90vh]">
            ...
            <div className="overflow-y-auto flex-1 px-4 py-3 space-y-3">
              ...
              <div className="grid grid-cols-2 gap-2">
                <div>
                  ...
                  <input
                    ...
                    placeholder="Restaurant, Real Estate…"
              ...
              <div>
                ...
                <div className="space-y-1.5">
                  {[
                    { key: 'instagram_url', label: 'Instagram', color: 'text-pink-500 focus:ring-pink-200', ph: 'instagram.com/username' },
                    { key: 'youtube_url',   label: 'YouTube',   color: 'text-red-500 focus:ring-red-200',   ph: 'youtube.com/@channel' },
                    { key: 'facebook_url',  label: 'Facebook',  color: 'text-blue-600 focus:ring-blue-200',  ph: 'facebook.com/page' },
                    { key: 'website_url',   label: 'Website',   color: 'text-stone-500 focus:ring-stone-200', ph: 'theirbusiness.com' },
                    { key: 'other_links',   label: 'Other',     color: 'text-violet-500 focus:ring-violet-200', ph: 'Any other link…' },
                  ].map(({ key, label, color, ph }) => (
                    <div key={key} className="flex items-center gap-2">
                      <span className={`text-[10px] font-bold w-16 flex-shrink-0 ${color.split(' ')[0]}`}>{label}</span>
                      <input
                        value={(form as any)[key]}
                        onChange={e => setForm(f => ({ ...f, [key]: e.target.value }))}
                        placeholder={ph}
                        className={`flex-1 px-2.5 py-1 text-[11px] border border-stone-200 rounded focus:outline-none focus:ring-1 ${color.split(' ').slice(1).join(' ')}`}
                      />
                    </div>
                  ))}
                </div>
            ...
            <div className="flex justify-end gap-2 px-4 py-3 border-t border-stone-100 flex-shrink-0">
              <button
