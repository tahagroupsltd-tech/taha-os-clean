  ...
  return (
    <div className="divide-y divide-stone-50">
      ...
      {recentTasks.map((task: any) => {
        const colors = mounted ? getTaskUrgencyColors(task.deadline, task.status as TaskStatus, now) : null
        return (
          <div key={task.id} className="px-4 py-3 flex items-start gap-3">
            ...
            <div className="text-right flex-shrink-0">
              ...
              {task.deadline ? (
                <span className={cn("inline-block text-[10px] font-medium px-2 py-0.5 rounded-full border mt-0.5", colors?.badgeClasses || 'bg-stone-50 text-stone-600 border-stone-100')}>
