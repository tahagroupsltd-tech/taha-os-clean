...
import { ArrowLeft, CheckSquare, FileVideo, Calendar, User as UserIcon, FolderOpen, CalendarRange } from 'lucide-react'
  ...
  return (
    <div className="flex flex-col h-full overflow-hidden">
      ...
      <div className="flex-1 overflow-y-auto p-5 space-y-5">
        {/* Project meta card */}
        <div className="bg-white rounded-lg border border-stone-100 p-5 shadow-card">
        ...
        >
          {project.tasks.map((t) => (
            ...
            >
              ...
              <Badge className={taskStatusColor(t.status as TaskStatus)}>
                {t.status.replace('_', ' ')}
              </Badge>
            </div>
          ))}
        ...
        <Section
          ...
          count={project.content.length}
          empty="No content on this project yet"
        >
              ...
              <div className="flex-1 min-w-0">
                ...
                <div className="flex items-center gap-2 text-[10px] text-stone-400 mt-0.5">
                  ...
                  {c.postDate && (
                    <>
                      <span>•</span>
                      <span>{formatDate(c.postDate)}</span>
                    </>
                  )}
                </div>
              </div>
              <Badge className={contentStatusColor(c.status as ContentStatus)}>
                {c.status}
              </Badge>
            </div>
          ))}
...
function MetaCell({
  ...
  return (
    <div>
      <div className="flex items-center gap-1 text-[10px] font-medium text-stone-400 uppercase tracking-wide">
        {icon} {label}
      </div>
